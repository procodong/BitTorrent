...
using BitTorrentClient.Core.Presentation.UdpTracker;
...
public sealed class UdpTrackerFetcher : ITrackerFetcher
{
    ...
    {
        var response = await ReadResponseAsync(() => SendConnectAsync(cancellationToken), cancellationToken);
    ...
    private async Task<IBufferReader> ReadResponseAsync(Func<Task<int>> send, CancellationToken cancellationToken = default)
    {
        var transactionId = await send();
        var resends = 0;
        while (true)
        {
            var receiveTask = _client.ReceiveAsync(cancellationToken).AsTask();
