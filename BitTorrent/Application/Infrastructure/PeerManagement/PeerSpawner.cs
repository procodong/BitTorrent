...
public class PeerSpawner
{
    ...
    private readonly ChannelWriter<RespondedPeerHandshaker> _peerAdderWriter;
    private readonly byte[] _peerId;

    public PeerSpawner(Download download, ILogger logger, ChannelWriter<int?> peerRemovalWriter, ChannelWriter<RespondedPeerHandshaker> peerWriter, byte[] peerId)
    {
        _download = download;
        _logger = logger;
    ...
    public async Task SpawnConnect(PeerAddress address)
    {
        try
        {
            ...
            var peerStream = new PeerHandshaker(stream, new BufferCursor(1 << 14 + 13));
            ...
            if (!responded.ReceivedHandshake.InfoHash.SequenceEqual(_download.Torrent.OriginalInfoHashBytes))
            {
    ...
    public async Task SpawnListener(RespondedPeerHandshaker handshaker, int index, PeerState state, ChannelReader<PeerRelation> relationReader, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                await handshaker.SendHandShakeAsync(new(0, _download.Torrent.OriginalInfoHashBytes, _peerId), cancellationToken);
                await handshaker.SendBitfieldAsync(_download.DownloadedPieces, cancellationToken);
            }
